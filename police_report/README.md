...
| **sign** | Per-request signature the mgop gateway validates (MD5 over request fields including `ts`). Computed inside the mgop native SDK (`libmsaoaidsec.so` / `libalisecuritysdk.so`); the embedded key is **not** reversed, so we capture and replay it. |

...
- **`sign`** is `GatewayUtil.md5Sign` (pure Python in `signer.OfflineMgopSigner`):
  `MD5("{secret}&api={api}&sid={gsid}&ts={ts}&data={base64(body)}")`.
  Still need a live portal **`gsid`**; no mitm capture needed to refresh `x-token`.

### Replay refresh — short window, not indefinite

A captured `(gsid, sign, ts)` triple can mint an x-token only while MGOP still
accepts that signed timestamp. Save it once as `.auth_replay.json`:

```bash
# auth-only capture (recommended):
MITM_AUTH_ONLY=1 mitmproxy -s mitm_addon.py
...
# when throttled cooldown blocks you and you know what you're doing:
python -m police_report.cli refresh --bypass-backoff
```
...
| `.auth_replay.json` (gsid expired) | ❌ no — replay returns `AUTH_EXPIRED` | ❌ no |
...
`cli refresh` prints `AUTH_EXPIRED`,
`.token_state.json` records `"outcome": "AUTH_EXPIRED"`, and the cron exits
non-zero. That's the signal to re-login + re-capture.
...
- **Lazy** — returns the cached JWT while it has >~10 min of life. Near expiry it
  ...
  unattended renewal.
- **Strict TTL on writes** — `upload` / `submit` call
  `get_token(min_ttl_s=600, allow_degraded=False)` and fail closed if refresh
  cannot produce a token with that much life. Reads may degrade to a still-valid
  cached JWT.
- **Auth-only retries** — idempotent reads refresh-and-retry **once** only when
  `WfjbError.auth_failure` (HTTP/business 401/403). Business/5xx errors do not
  burn a replay. `submit` refreshes *before* filing and **never** retries after.
- **Explicit outcomes** — `OK` / `SIGNATURE_EXPIRED` / `AUTH_EXPIRED` /
  `THROTTLED` / `UPSTREAM_ERROR` / `PROTOCOL_ERROR` / `GATEWAY_ERROR` /
  `NETWORK_ERROR` / `BAD_TEMPLATE`, recorded in `.token_state.json`.
- **Throttle circuit breaker** — HTTP 429 sets `next_retry_at` (5→15→30→60 min +
  jitter). Ordinary `refresh` / cron / `get_token` respect it;
  `refresh --bypass-backoff` overrides.
- **Atomic token store** — sole source is `.token.json` (mode `0600`). `.env` holds
  identity (`WFJB_PHONE` / `WFJB_NAME`) and optional one-shot overrides only —
  refresh never writes `WFJB_X_TOKEN` into `.env`. State write is best-effort and
  never blocks saving a newly minted token.
...
### Headless mint (no physical phone)

Stale `.auth_replay.json` cannot be repaired by waiting (`rs=7003`). Configure a
`TokenMinter` so `refresh` falls back after `SIGNATURE_EXPIRED` / `BAD_TEMPLATE`:

| `WFJB_MINTER` | Behavior |
|---|---|
| unset / `off` | replay only |
| `offline` | `OfflineMgopSigner` (pure Python) + gsid → `mgop_auth` |
| `signer` | `BridgeMgopSigner` → `MGOP_SIGNER_URL` + `mgop_auth` |
| `android` | boot AVD, mitm capture (legacy) |

```bash
# headless mint (needs live portal gsid in .session.json or .auth_replay.json sid)
WFJB_MINTER=offline python -m police_report.cli mint --via offline

# or let refresh auto-fallback when replay signature expires
export WFJB_MINTER=offline
python -m police_report.cli refresh
```

Interfaces: `police_report/signer.py` (`OfflineMgopSigner`), `police_report/minter.py`.
Portal **gsid** still expires separately — renew via `cli login` / app login, then mint.
...
```bash
MITM_AUTH_ONLY=1 mitmproxy -s mitm_addon.py   # disk: wfjb.auth only, mode 0600
python -m police_report.cli save-replay /tmp/re/mitm.jsonl
python -m police_report.cli refresh
python -m police_report.cli whoami
# then delete the raw capture
```
...
- `--x-token …` / `WFJB_X_TOKEN=…` — paste an `X-Token` value from a live session
  (explicit override; not written back by `refresh`).
...
| file | holds | secret? |
|---|---|---|
| `.auth_replay.json` | the frozen `(gsid, sign, ts)` mint request | **yes** |
| `.token.json` | sole token store: `x_token` + `cna` + `exp` | **yes** |
| `.env` | reporter identity (`WFJB_PHONE`/`WFJB_NAME`); optional overrides | **yes** |
| `.session.json` | portal login result from `cli login` (gsid/tokens) | **yes** |
| `.token_state.json` | last outcome, `next_retry_at`, counters (no secrets) | no |

`.env` is auto-loaded by the CLI. Authoritative refresh material is
`.auth_replay.json`; the live JWT lives only in `.token.json`.
...
| term | meaning |
|---|---|
| **gsid** | "global session id" — portal SSO session token/cookie the `警察叔叔` app gets when a human logs in (phone+SMS, Alipay, or gov SSO). Long-lived (days–weeks). **Not** minted by this client; captured from a live session. |
| **mgop** | "mobile gateway open platform" — Alibaba mPaaS's RPC protocol. Every native-API call goes through `POST /app/mgop` with header `api: mgop.<vendor>.<app>.<action>`; ours is `mgop.trustway.wfjb.auth`. The body and headers are signed by a native SDK. |
| **mPaaS** | Alibaba's mobile-PaaS umbrella product — the framework `com.hzpd.jwztc` is built on. Owns mgop + the hardening native libs. |
| **wfjb** | 违法举报 (wéifǎ jǔbào, "traffic-violation report") — the reporting feature. Appears as the path component `wfjb-front-api` and inside the mgop API name `…wfjb.auth`. |
| **jwztc** | 警察叔叔 ("police uncle") — Android package `com.hzpd.jwztc`. App v3.14.26 was reverse-engineered to build this client. |
| **x-token** | The ~1 h JWT (`exp` claim) every wfjb API call needs, sent as `X-Token: …`. Minted by the mgop `wfjb.auth` call; refreshed by replay. |
| **cna** | Persistent device cookie (Umeng-style install id) required alongside the x-token. |
| **sign** | `GatewayUtil.md5Sign`: `MD5(secret&api=&sid=&ts=&data=b64(body))`. Offline in `signer.py`. |
| **ts** | Epoch-ms timestamp the `sign` is bound to. Once aged, the gateway rejects the replay with header `rs=7003` (验签时间戳校验失败) — recoverable only by a fresh mitm capture. |
| **rs** | Response-status code emitted by mgop in **response headers**, not the JSON body. `7003` = signed-timestamp expired, `4001` = gateway timeout. The body is often empty. |
| **JWT** | JSON Web Token — the format of the x-token. |
| **SSO** | Single sign-on — the portal login flow the gsid is bound to. |
| **H5** | The mobile webview (HTML5 page) inside the app that calls the wfjb API. |
| **MITM** | Man-in-the-middle proxy (mitmproxy in this repo) used to capture traffic from the rooted phone. |
| **WAF** | Web Application Firewall — gov-side geo-DNS that returns China-only IPs; the capture host must egress China-local or the gateway is unreachable. |
| **DenyList** | Magisk/Zygisk feature that hides root from a named app so the anti-root check in `com.hzpd.jwztc` passes. |
| **OCR / tesseract / chi_sim** | OCR pipeline (`extract_overlay.py`) — open-source Tesseract engine + Simplified-Chinese language model, used to read the dashcam overlay. |
| **DVR** | Digital Video Recorder — dashcam. The input source for the OCR pipeline. |
| **SDK** | Software Development Kit — here, the mgop native libs `libmsaoaidsec.so` / `libalisecuritysdk.so` inside the app. |
